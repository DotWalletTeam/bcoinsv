    ...
    for (let ip = 0; ip < this.code.length; ip++) {
      ...
      switch (op.value) {
        ...
        case opcodes.OP_MAX: {
          ...
          switch (op.value) {
            ...
            case opcodes.OP_MUL:
              ...
              if (!(flags & Script.flags.VERIFY_OPMUL))
                throw new ScriptError('BAD_OPCODE', op, ip);
              num = n1.imul(n2);
        ...
        case opcodes.OP_LSHIFT: {
          // Make sure this remains an error before activation.
          if (!(flags & Script.flags.VERIFY_OPLSHIFT))
            throw new ScriptError('BAD_OPCODE', op, ip);

          const n = stack.getNum(-1, minimal, 4).toInt();
          stack.pop();
          if (n < 0)
            throw new ScriptError('INVALID_NUMBER_RANGE', op, ip);

          const shifted = Buffer.from(stack.pop());
          BufferShift.shl(shifted, n);
        ...
        case opcodes.OP_RSHIFT: {
          // Make sure this remains an error before activation.
          if (!(flags & Script.flags.VERIFY_OPRSHIFT))
            throw new ScriptError('BAD_OPCODE', op, ip);
        ...
        case opcodes.OP_INVERT: {
          // Make sure this remains an error before activation.
          if (!(flags & Script.flags.VERIFY_OPINVERT))
            throw new ScriptError('BAD_OPCODE', op, ip);

          const n = stack.pop();
          const inverted = Buffer.from(n);
