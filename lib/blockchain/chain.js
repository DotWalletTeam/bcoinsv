...
class Chain extends AsyncEmitter {
  ...
  async getDeployments(time, prev) {
    ...
    if (mtp >= this.network.block.magneticAnomalyActivationTime) {
      ...
      state.flags |= Script.flags.VERIFY_OPMUL;
      state.flags |= Script.flags.VERIFY_OPINVERT;
      state.flags |= Script.flags.VERIFY_OPLSHIFT;
      state.flags |= Script.flags.VERIFY_OPRSHIFT;
    }
  ...
  async verifyInputs(block, prev, state) {
    ...
    // CTOR was not enabled on the BSV chain.
    const checkTTOR = true;
